...
(defun best-path-3 (grid start points)
  (bind ((xs   (make-instance 'cl-heap:priority-queue))
         (best (make-hash-table :test #'equal))
         (fin  points)
         (in-shortest (make-hash-table :test #'equal)))
    (labels ((grid-aref (c)
               (aref (aref grid (imagpart c))
                     (realpart c)))
             (consider-for-queue (c dir dist path)
               ;; (format t "consider: (list c dir dist): ~a~%" (list c dir dist))
               (bind ((nc (+ c dir))
                      (n-dist (+ dist 1)))
                 (when (not (char-equal #\# (grid-aref nc)))
                   (bind ((key (cons nc dir))
                     ...
                     (when (or (null b) (>= b n-dist))
                       ;; (format t "Enqueued~%")
                       (setf (gethash key best) n-dist)
                       (cl-heap:enqueue xs (list nc dir n-dist (cons nc path)) n-dist))))))
             (recur ()
               (bind ((next (cl-heap:dequeue xs)))
                 (when next
                   (bind (((c dir dist path) next)
                          (b (gethash (cons c dir) best))
                          (s (grid-aref c)))
                     ;; (format t "recur: (list c dir dist): ~a~%" (list c dir dist))
                     (cond
                       ((char-equal s #\E) (iter
                                             (for p in path)
                                             (setf (gethash p in-shortest) t)))
                       ((> dist fin) t)
                       ((= dist b)
                        (bind ((l (* dir #c(0 1)))
                               (r (* dir #c(0 -1))))
                          (consider-for-queue c l (+ dist 1000) path)
                          (consider-for-queue c r (+ dist 1000) path)
                          (consider-for-queue c dir dist path))))
                     (recur))))))
      (cl-heap:enqueue xs (list start #c(1 0) 0 (list start)) 0)
      (setf (gethash (cons start #c(1 0)) best) 0)
      (recur)
      in-shortest)))
...
(defun part-2 ()
  (bind ((grid (read-problem))
    ...
    (hash-table-count (best-path-3 grid start (best-path grid start)))))
